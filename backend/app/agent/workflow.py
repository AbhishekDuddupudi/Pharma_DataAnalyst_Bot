...
class WorkflowState:
    ...
    # Observability — Langfuse trace ID set by run_workflow via @observe
    trace_id: str = ""
...
async def _preprocess_input(state: WorkflowState, emit: Emitter) -> None:
    ...
    lf = get_client()
    with lf.start_as_current_observation(as_type="span", name="preprocess_input") as span:
        span.update(input={"message": state.user_message[:500]})
        ...
        span.update(output={"mode": state.mode, "preprocessed_len": len(msg)})
...
async def _scope_policy_check(state: WorkflowState, emit: Emitter) -> None:
    ...
    """
    lf = get_client()
    span = lf.start_observation(name="scope_policy_check", as_type="span")
    span.update(input={"preprocessed": state.preprocessed[:300]})
    ...
    for pat in _BLOCKED_PATTERNS:
        if pat.search(lower):
            ...
            span.update(output={"result": "blocked_rules"}, level="WARNING"); span.end()
            return
    ...
    if intent and (words & _FOLLOW_UP_WORDS):
        ...
        span.update(output={"result": "allowed_followup"}); span.end()
        return
    ...
    if words & greeting_words and (len(words) < 6 or words & bot_words):
        ...
        span.update(output={"result": "allowed_greeting"}); span.end()
        return
    ...
    if combined_score >= 2:
        ...
        if state.mode == "insights":
            ...
            if len(missing) >= 2:
                ...
                span.update(output={"result": "needs_clarification", "missing": missing}); span.end()
                return
        ...
        span.update(output={"result": "allowed_domain", "domains": list(domain_overlap)[:10]}); span.end()
        return
    ...
    if not result.get("in_scope", True):
        ...
        span.update(output={"result": "rejected_llm", "reason": state.reject_reason[:100]}, level="WARNING"); span.end()
    else:
        await emit("status", {
            "step": "scope_policy_check",
            "message": "Allowed (LLM)",
        })
        span.update(output={"result": "allowed_llm"}); span.end()
...
async def _sql_generator(state: WorkflowState, emit: Emitter) -> None:
    ...
    lf = get_client()
    with lf.start_as_current_observation(as_type="span", name="sql_generator") as span:
        span.update(input={"tasks_count": len(state.tasks)})
        schema_text = _schema_summary()
        ...
        for task in state.tasks:
            ...
            with lf.start_as_current_observation(as_type="span", name=f"sql_gen.{task.title[:40]}") as task_span:
                task_span.update(input={"task": task.title})
                ...
                task_span.update(output={"sql_chars": len(task.sql)})

        span.update(output={"tasks_generated": len(state.tasks)})
...
async def _sql_validator(state: WorkflowState, emit: Emitter) -> None:
    ...
    lf = get_client()
    with lf.start_as_current_observation(as_type="span", name="sql_validator") as span:
        span.update(input={"tasks_count": len(state.tasks)})
        ...
        for task in state.tasks:
            ...
            if not result.valid:
                ...
                task.error = None
        span.update(output={"invalid_count": invalid_count})
...
async def _sql_repair(state: WorkflowState, emit: Emitter) -> None:
    ...
    lf = get_client()
    with lf.start_as_current_observation(as_type="span", name="sql_repair") as span:
        span.update(input={"tasks_needing_repair": len(needs_repair)})
        schema_text = _schema_summary()
        policy = get_allowlist_summary()
        total_retries = 0
        ...
        span.update(output={"total_retries": total_retries, "still_invalid": sum(1 for t in needs_repair if not t.valid)})
...
async def _sql_executor_node(state: WorkflowState, emit: Emitter) -> None:
    ...
    lf = get_client()
    with lf.start_as_current_observation(as_type="span", name="sql_executor") as span:
        span.update(input={"tasks_count": len(state.tasks)})
        ...
        span.update(output={"db_ms": state.db_ms, "rows_returned": state.rows_returned})
...
async def _response_synthesizer(state: WorkflowState, emit: Emitter) -> None:
    ...
    lf = get_client()
    with lf.start_as_current_observation(as_type="span", name="response_synthesizer") as span:
        ...
        span.update(output={"answer_len": len(answer), "tokens_streamed": state.tokens_streamed})


# ── Main orchestrator ────────────────────────────────────────────


@observe(name="chat.stream", capture_input=False, capture_output=False)
async def run_workflow(
    ...
    *,
    ...
    Parameters:
        ...
        user_id       – authenticated user ID (propagated to Langfuse).
        session_id    – chat session ID (propagated to Langfuse).

    Returns the final WorkflowState with all results.
    """
    lf = get_client()
    # Propagate user/session attributes to all child observations so
    # Langfuse can group by user and session in the dashboard.
    with propagate_attributes(
        user_id=str(user_id) if user_id is not None else None,
        session_id=str(session_id) if session_id is not None else None,
        trace_name="chat.stream",
    ):
        lf.update_current_span(input={"message": user_message[:500]})
        state = WorkflowState(
        ...
        state.trace_id = lf.get_current_trace_id() or ""
